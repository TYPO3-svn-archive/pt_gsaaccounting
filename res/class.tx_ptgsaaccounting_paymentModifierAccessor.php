...
***************************************************************/
/** 
 * Database accessor class for payment Modifiers of the 'pt_gsaaccounting' extension
 *
 * $Id$
 *
...
/**
 * payment Modifier Database Accessor class 
 *
 * @author      Dorit Rottner <rottner@punkt.de>
 * @since       2008-11-21
 * @package     TYPO3
 ...
 */
class tx_ptgsaaccounting_paymentModifierAccessor  implements tx_pttools_iSingleton {
    /*
     * Contansts
     */
    const DB_TABLE = 'tx_ptgsaaccounting_paymentModifier';
     ...
     *
     * @param   void
     * @return  tx_ptgsaaccounting_paymentModifierAccessor      unique instance of the object (Singleton) 
     * @author  Dorit Rottner <rottner@punkt.de>
     * @since   2008-11-21
     */
    ...
    /**
     * Returns an array with Ids payment Modifier records with orderUid
     *
     * @param   integer     order Uid
     * @return  array       array with selectes record IDs 
     * @throws  tx_pttools_exception if the query fails/returns false
     * @author  Dorit Rottner <rottner@punkt.de>
     * @since   2008-11-21
     */
    public function getIdArr($orderUid=0){
        ...
        $where = '1';
        if ($orderUid) {
            $where .= ' AND orderUid = ' . intval($orderUid);
        }
    ...
    public function storePaymentModifierData($dataArr) {
        trace('[METHOD] '.__METHOD__);
        $row = array (
               'pid' => $conf['accountingDataSysfolderPid'],
               'uid' => $dataArr['uid'],
               'orderUid' => intval($dataArr['orderUid']),
               'addDataType' => (string)$dataArr['addDataType'],
               'addData' => (string)$dataArr['addData'],
               'value' => (double)$dataArr['value'],
        );

        if (intval($dataArr['uid']) == 0) {
            $conf = tx_pttools_div::typoscriptRegistry('config.tx_ptgsaaccounting.');
        	$row['pid'] = $conf['accountingDataSysfolderPid'];
    ...
    public function updateRecord($fieldsArr) {
        ...
        if ($result == false) {
            throw new tx_pttools_exception('Error in Update pt_gsaacounting_paymentModifier', 1);
        }
    }


    /**
     * Select Data from Database Record with given uid
    ...
    public function selectPaymentModifierByUid($uid) {
        trace('METHOD] '.__METHOD__);
        // query preparation
    ...
    public function selectRecord($where, $groupBy='', $orderBy='', $limit='' ) {
        ...
        trace($a_row,0,'paymentModifierArray'); 
        return $a_row;
    }
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pt_gsaaccounting/res/class.tx_ptgsaaccounting_paymentModifierAccessor.php']) {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pt_gsaaccounting/res/class.tx_ptgsaaccounting_paymentModifierAccessor.php']);
}
?>
